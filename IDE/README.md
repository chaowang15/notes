...
C++ code formater based on ClangFormat, for beautiful code with minimal effort in Sublime Text 3.
...
To use the custom clang-format file, firstly install the package 'Clang format', and set the path of the `.clang-format` file by 'Clang Format: set path', and then 'Clang Format: select style' and choose 'file'. Finally select some code and then 'Clang Format: Format Selection' to format the code.
...
Index code symbols for code naviation. It's very useful and necessary for variable and function navigation and will add command 'Go/Jump/Navigation to definition' in right clicking menu. After installation, run 'ctags: rebuild ctags' and it will build and create a ctags file '.ctags' in project folder.
...
Default C/C++ syntax theme is very bad that it can hardly detect many types.
We can improve it by installing new package and adding some new definition inside it.
- Firstly install package 'C++ Starting Kit'. This new syntax is better than default C++ syntax theme that almost all built-in keywords are detected (like namespace, string, etc). However, one important missing type is the Class/Struct keyword. So the idea is to change some content of existing 'C++ Starting Kit' package.
- Create a copy of 'C++ Starting Kit'. Firstly install package 'PackageResourceViewer' which is used to extract package files. After installation, run 'PackageResourceViewer: extract packages' to extract package files of 'C++ Starting Kit'. It will create a folder inside sublime text as `~/.config/sublime-text3/Packages/C++ Starting Kit`. You can rename the folder as a new one like 'CNew' to distinguish between it and original package. Now you can find the new syntax name CNew in 'View -> Syntax' in sublime text 3 and its functionality now is exactly the same as 'C++ Starting Kit'.
- Change the file `~/.config/sublime-text3/Packages/User/Package Control.sublime-settings` by adding the new package name 'CNew' inside the `installed_packages`. If you don't do this, the extracted files will be **deleted automatically after you close sublime text**.
...
```
Make a **copy** underneath and change it like this:
```
{
    "name": "Duncan",
